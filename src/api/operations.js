...
export function useSaveAttendance() { const client = useQueryClient(); return useMutation({ mutationFn: (value) => put(`/Operations/attendance/${value.shiftId}`, { ...value, clockIn: asOrganizationInstant(value.clockIn), clockOut: asOrganizationInstant(value.clockOut) }), onSuccess: () => client.invalidateQueries({ queryKey: ["operations", "attendance"] }) }); }
